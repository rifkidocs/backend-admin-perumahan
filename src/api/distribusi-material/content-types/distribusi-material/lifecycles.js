module.exports = {
  async beforeCreate(event) {
    ...
    if (data.material && data.gudang_asal && data.jumlah) {
      const material = await strapi.entityService.findOne('api::material.material', data.material, {
      ...
      if (material.lokasi_gudang && material.lokasi_gudang.id !== data.gudang_asal) {
      ...
      if (material.stok < data.jumlah) {
        throw new Error(`Stok tidak mencukupi di gudang asal. Stok tersedia: ${material.stok}`);
      }
    }
  },
  ...
  async afterUpdate(event) {
    ...
    if (data.status === 'Diterima' && result.status === 'Diterima') {
      const distribusi = await strapi.entityService.findOne('api::distribusi-material.distribusi-material', result.id, {
      ...
      if (sourceMaterial) {
        await strapi.entityService.update('api::material.material', sourceMaterial.id, {
          data: {
      ...
      const destMaterials = await strapi.entityService.findMany('api::material.material', {
        filters: {
      ...
      if (destMaterials && destMaterials.length > 0) {
        // Update existing material
        const destMaterial = destMaterials[0];
        await strapi.entityService.update('api::material.material', destMaterial.id, {
          data: {
