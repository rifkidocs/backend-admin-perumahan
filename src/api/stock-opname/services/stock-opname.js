...
module.exports = createCoreService('api::stock-opname.stock-opname', ({ strapi }) => ({
  async generateOpnameNumber() {
    ...
    const lastOpname = await strapi.entityService.findMany('api::stock-opname.stock-opname', {
      filters: {
        opname_number: {
  ...
  async getStockOpnameSummary(stockOpnameId) {
    ...
    const varianceItems = items.filter(item => item.difference !== 0).length;
    ...
    const totalValueOver = items
      .filter(item => item.difference > 0 && item.material?.harga_satuan)
      .reduce((sum, item) => sum + (item.difference * item.material.harga_satuan), 0);

    const totalValueShort = items
      .filter(item => item.difference < 0 && item.material?.harga_satuan)
      .reduce((sum, item) => sum + (Math.abs(item.difference) * item.material.harga_satuan), 0);
    ...
    return {
      ...
      summary: {
        ...
        totalValueOver,
        totalValueShort,
        matchRate: totalItems > 0 ? ((matchedItems / totalItems) * 100).toFixed(2) : 0
      }
    };
  },
  ...
  async completeStockOpname(stockOpnameId, reviewerId) {
    ...
    if (!stockOpname.stock_opname_items || stockOpname.stock_opname_items.length === 0) {
      throw new Error('Cannot complete Stock Opname without items');
    }

    // Update status and add reviewer
    const updatedStockOpname = await strapi.entityService.update('api::stock-opname.stock-opname', stockOpnameId, {
      data: {
        status: 'Completed',
        reviewer: reviewerId
      }
    ...
    // Log completion
    strapi.log.info(`Stock Opname ${updatedStockOpname.opname_number} completed by reviewer ID: ${reviewerId}`);
  ...
  async getVarianceAnalysis(stockOpnameId) {
    ...
    items.forEach(item => {
      const category = item.material?.kategori || 'Unknown';
      ...
      if (item.material?.harga_satuan) {
        analysis.categories[category].totalValue += Math.abs(item.difference) * item.material.harga_satuan;
      }
    });

    // Generate recommendations
    if (analysis.overages.length > 0) {
      analysis.recommendations.push('Review receiving procedures for materials with consistent overages');
    }
    if (analysis.shortages.length > 0) {
      analysis.recommendations.push('Investigate potential causes of material shortages or theft');
    }
    if (items.length > 0) {
      analysis.recommendations.push('Consider implementing more frequent stock checks for high-variance materials');
    }
